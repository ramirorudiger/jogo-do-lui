"""Lui - Shih Tzu realista, com esqueleto e animações (Andar, Parado, Latir).
Blender 4.x/5.x: aba Scripting > Open > lui_animado.py > Run Script (leva ~2 min).
"""
import bpy, math, sys
import numpy as np
...
N_FIOS = int(next((a.split("=")[1] for a in sys.argv if a.startswith("fios=")), 150000))
...
# malha mais leve (boa para animar e para o jogo)
_ratio = min(1.0, 14000 / max(1, len(lui.data.polygons)))
dm = lui.modifiers.new("Decimate", 'DECIMATE'); dm.ratio = _ratio
bpy.ops.object.modifier_apply(modifier="Decimate")
bpy.ops.object.shade_smooth()
bpy.ops.object.mode_set(mode='EDIT'); bpy.ops.mesh.select_all(action='SELECT')
bpy.ops.uv.smart_project(island_margin=0.004); bpy.ops.object.mode_set(mode='OBJECT')
print("faces:", len(lui.data.polygons))
...
for i, c in enumerate(EYES):
    ...
    M, g = eye_frame(sx)
    # globo
    bpy.ops.mesh.primitive_uv_sphere_add(radius=EYE_R, segments=48, ring_count=24, location=(0, 0, 0))
    ...
    bpy.ops.mesh.primitive_uv_sphere_add(radius=EYE_R*1.012, segments=48, ring_count=24, location=(0, 0, 0))
    ...
    bpy.ops.mesh.primitive_uv_sphere_add(radius=EYE_R*1.02, segments=64, ring_count=32, location=(0, 0, 0))
...
ti_k = ti[keep][:N_FIOS]
bary = np.stack([u, v, w], 1)[keep][:N_FIOS]
N = len(root)
...
# coordenada UV de cada raiz (prende o fio na pele para acompanhar a animação)
uvl = np.zeros((len(me.loops), 2)); me.uv_layers.active.data.foreach_get("uv", uvl.ravel())
tl = tloops[ti_k]
root_uv = (bary[:, 0:1]*uvl[tl[:, 0]] + bary[:, 1:2]*uvl[tl[:, 1]] + bary[:, 2:3]*uvl[tl[:, 2]]).astype(np.float32)
suv = hc.attributes.new("surface_uv_coordinate", 'FLOAT2', 'CURVE')
suv.data.foreach_set("vector", root_uv.ravel())
hc.surface = lui
hc.surface_uv_map = me.uv_layers.active.name
pelo.parent = lui

# comprimento do pelo por vértice (usado pelo jogo, guardado no alfa da cor)
kd2 = KDTree(N)
for i, p in enumerate(root): kd2.insert(p, i)
kd2.balance()
for vv in me.vertices:
    near8 = kd2.find_n(vv.co, 8)
    ln = sum(L[j] for _, j, _ in near8) / len(near8)
    c = attr.data[vv.index].color
    attr.data[vv.index].color = (c[0], c[1], c[2], min(1.0, ln / 0.12))
...
# ---------- ESQUELETO ----------
arm_data = bpy.data.armatures.new("EsqueletoLui")
arm = bpy.data.objects.new("EsqueletoLui", arm_data)
...
def bone(name, h, t, parent=None, connect=False):
    b = eb.new(name); b.head = h; b.tail = t; b.roll = 0
    if parent: b.parent = eb[parent]; b.use_connect = connect
    return b
bone("quadril", (0, 0.40, 0.46), (0, 0.12, 0.47))
bone("peito", (0, 0.12, 0.47), (0, -0.17, 0.50), "quadril", True)
bone("pescoco", (0, -0.17, 0.50), (0, -0.30, 0.68), "peito", True)
bone("cabeca", (0, -0.30, 0.68), (0, -0.40, 0.93), "pescoco", True)
for s, sd in ((1, "L"), (-1, "R")):
    bone(f"orelha.{sd}", (s*0.18, -0.30, 0.84), (s*0.20, -0.30, 0.56), "cabeca")
    bone(f"braco.{sd}", (s*0.115, -0.20, 0.40), (s*0.115, -0.20, 0.20), "peito")
    bone(f"pata_f.{sd}", (s*0.115, -0.20, 0.20), (s*0.115, -0.22, 0.02), f"braco.{sd}", True)
    bone(f"coxa.{sd}", (s*0.125, 0.36, 0.42), (s*0.125, 0.39, 0.20), "quadril")
    bone(f"pata_t.{sd}", (s*0.125, 0.39, 0.20), (s*0.125, 0.38, 0.02), f"coxa.{sd}", True)
bone("rabo1", (0, 0.52, 0.56), (0, 0.55, 0.72), "quadril")
bone("rabo2", (0, 0.55, 0.72), (0, 0.44, 0.80), "rabo1", True)
bpy.ops.object.mode_set(mode='OBJECT')

# pele com pesos automáticos
bpy.ops.object.select_all(action='DESELECT')
lui.select_set(True); arm.select_set(True); bpy.context.view_layer.objects.active = arm
bpy.ops.object.parent_set(type='ARMATURE_AUTO')

# olhos, nariz e gravatinha viram parte da mesma malha, presos ao osso certo
extras = [(o, "cabeca") for o in scene.objects if o.name.startswith(("Olho", "Cornea", "Palpebra", "Nariz"))]
for o, _ in extras:                       # aplica espessura/suavização das pálpebras antes de unir
    bpy.context.view_layer.objects.active = o
    for m in list(o.modifiers): bpy.ops.object.modifier_apply(modifier=m.name)
extras.append((bow, "pescoco"))
for o, bn_ in extras:
    vg = o.vertex_groups.new(name=bn_)
    vg.add(list(range(len(o.data.vertices))), 1.0, 'REPLACE')
    if o.data.uv_layers:          # UV fora da área da pele, para não confundir o pelo
        o.data.uv_layers[0].name = lui.data.uv_layers.active.name
        for d_ in o.data.uv_layers[0].data: d_.uv.x += 2.0
    for m in o.modifiers: pass
bow_sub = bow.modifiers.get("Sub")
if bow_sub:
    bpy.context.view_layer.objects.active = bow; bpy.ops.object.modifier_apply(modifier="Sub")
bpy.ops.object.select_all(action='DESELECT')
for o, _ in extras: o.select_set(True)
lui.select_set(True); bpy.context.view_layer.objects.active = lui
bpy.ops.object.join()

# pelo acompanha a pele durante a animação
gn = bpy.data.node_groups.new("PeloSegueAPele", 'GeometryNodeTree')
...
gi_ = gn.nodes.new("NodeGroupInput"); go_ = gn.nodes.new("NodeGroupOutput")
dc = gn.nodes.new("GeometryNodeDeformCurvesOnSurface")
gn.links.new(gi_.outputs[0], dc.inputs[0]); gn.links.new(dc.outputs[0], go_.inputs[0])
pm = pelo.modifiers.new("PeloSegueAPele", 'NODES'); pm.node_group = gn
lui.add_rest_position_attribute = True
...
def make_action(name, length, fn, bones):
    ...
    for f in range(0, length + 1):
        ...
        fn(f / length)
        for bnm in bones:
            pb = arm.pose.bones[bnm]
            pb.keyframe_insert("rotation_euler", frame=f + 1)
            if bnm == "quadril": pb.keyframe_insert("location", frame=f + 1)
    return act
...
def andar(t):
    ...
    for sd, ph in (("L", 0.0), ("R", math.pi)):
        P[f"braco.{sd}"].rotation_euler.x = R(24) * math.sin(a + ph)
        P[f"pata_f.{sd}"].rotation_euler.x = R(-22) * max(0, math.sin(a + ph + 1.3))
        P[f"coxa.{sd}"].rotation_euler.x = R(-22) * math.sin(a + ph + math.pi)
        P[f"pata_t.{sd}"].rotation_euler.x = R(20) * max(0, math.sin(a + ph + math.pi + 1.3))
        P[f"orelha.{sd}"].rotation_euler.x = R(6) * math.sin(2 * a + 0.8)
    P["quadril"].location.z = 0.012 * math.sin(2 * a)
    P["quadril"].rotation_euler.y = R(3) * math.sin(a)
    P["rabo1"].rotation_euler.z = R(22) * math.sin(2 * a)
    P["rabo2"].rotation_euler.z = R(18) * math.sin(2 * a - 0.9)
walk_bones = ["quadril", "rabo1", "rabo2"] + [f"{b}.{s}" for b in ("braco", "pata_f", "coxa", "pata_t", "orelha") for s in "LR"]

def parado(t):
    a = 2 * math.pi * t
    P["rabo1"].rotation_euler.z = R(14) * math.sin(2 * a)
    P["rabo2"].rotation_euler.z = R(10) * math.sin(2 * a - 0.9)
    P["quadril"].location.z = 0.004 * math.sin(a)
    for sd in "LR": P[f"orelha.{sd}"].rotation_euler.x = R(2) * math.sin(a)

def latir(t):
    # dois latidos rápidos: cabeça sobe e volta, orelhas pulam
    k = max(0.0, math.sin(2 * math.pi * 2 * t)) * (1 - t * 0.3)
    P["pescoco"].rotation_euler.x = R(-14) * k
    P["cabeca"].rotation_euler.x = R(-10) * k
    for s, sd in ((1, "L"), (-1, "R")):
        P[f"orelha.{sd}"].rotation_euler.x = R(-16) * k
        P[f"orelha.{sd}"].rotation_euler.z = R(8) * k * s
bark_bones = ["pescoco", "cabeca", "orelha.L", "orelha.R"]

make_action("Andar", 24, andar, walk_bones)
make_action("Parado", 48, parado, walk_bones)
make_action("Latir", 14, latir, bark_bones)
for act in bpy.data.actions:
    for fc in getattr(act, "fcurves", []):
        fc.modifiers.new('CYCLES') if act.name != "Latir" else None
pose_reset()
arm.animation_data.action = bpy.data.actions["Andar"]
